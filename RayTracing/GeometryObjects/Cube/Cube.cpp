...
bool Cube::Hit(CRay const& ray, CIntersection& intersection) const
{
	...
	*/
	...
	// 
	// Точка, где луч пересекает куб
	CVector3d hitPoint = ray.GetPointAtTime(tmin);
	CVector3d hitPointInObjectSpace = invRay.GetPointAtTime(tmin);

	double epsilon = 0.000000000001;

	// Переменный абсолютных значений разниц между координатами. 
	// Помогают определить, на какой из граней куба произошло пересечение.
	...
	// Относительно того, с какой гранью произошло касание, выставляем нормаль к нужной грани
	...
	// Вычисляем нормаль к плоскости в мировой системе координат
